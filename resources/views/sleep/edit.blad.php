...
<div class="container">
    ...
    <form method="post" action="{{ route('sleep.update') }}">
        ...
        <div class="form-group">
            ...
            <input type="date" name="nap_start" id="nap_start" class="form-control" value="{{old('nap_start') }}" required>
            @error('nap_start')
            <p class="text-danger">{{ $message }}</p>
            @enderror
        </div>
      ...
      <div class="form-group">
        ...
        <input type="notes" name="notes" id="notes" class="form-control" value="{{old('notes')}}" required>
        @error('notes')
        <p class="text-danger">{{ $message }}</p>
        @enderror
      </div>
